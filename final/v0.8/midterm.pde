/**
 * @author Alex Rirak <arirak@nyu.edu>, Mile Divovic <md2950@nyu.edu>
 * @since 10/26/2015
 * This is a remake of the first level of Super mario Bros. created as my midterm project for Interactive Computing at NYU
 * Mile and I have added a level creator as our Final Project for Interactive Computing at NYU
 */
...
boolean invalidTextEntry = false;
...
void draw()
{
  if (state == 0) {
    ...
    background(0);
    textSize(30);
    text("Type the name of your level:", 0, 210, width, height);
    text(myText, 0, 250, width, height);
    textSize(14);
    text("(Press Space Bar to Save)", 0, 290, width, height);
    ...
    background(0);
    textSize(20);
    text("Type the name of the level you wish to play:", 10, 0, width, height);
    text(myText, 10, 40, width, height);
    textSize(14);
    text("(Press Space Bar to continue)", 10, 80, width, height);
    levelDB.getPossibleLevels();

    if(invalidTextEntry){
      fill(255, 0, 0);
      text("Invalid Level Entered", 10, 20, width, height);
    }
    fill(255);
...
void keyPressed()
{
  if(textEntry) {
    if (keyCode == BACKSPACE) {
      ...
      myText = "";
    } else if (keyCode != SHIFT && keyCode != CONTROL && keyCode != ALT && keyCode != ENTER && key != ' ') {
      myText = myText + key;
    } else if(key == ' ') {
      if(state == 7) {
        levelDB.saveNewLevel(myText);
        textEntry = false;
        levelDB.resetGame();
        state = 0;
      } else if (state == 8) {
        if(levelDB.checkExists(myText)) {
          invalidTextEntry = false;
          levelName = myText;
          soundDB.playLevelOneTheme();
          textEntry = false;
          levelDB.resetGame();
...
class BreakingBlock
{
  ...
  BreakingBlock(float _x, float _y, String _type)
  {
    ...
    BreakingBlock(float _x, float _y, String _type, String _color)
  {
    // store position
    x = _x;
    y = _y;
...
class Player
{
  ...
  void move()
  {
    ...
    if (keyD) 
    {
      ...
      if(x - X_OFFSET >= (level[0].length - 19) * CELL_SIZE){
        state = 5;
      }
    }
...
class menuDBClass {
  ...
  void mainMenu() {
    ...
    text("Game Remade by: Alex Rirak!", 175, 280);
    text("Level Creator by: Alex Rirak and Mile Divovic!", 175, 295);
    textSize(24);
    text("Press Enter to Begin!", 150, 350);
    text("Press C to enter level creator", 150, 380);
    textSize(14);
  ...
  void creatorMenu() {
    if (creatorMenuOn == true) {
      ...
      if (categoryMenu == true) {
        ...
        image(coinArt, 340, 10); tint(255, 150); image(questionBlock, 340, 10); tint(255, 255);
        image(mushroom, 370, 10); tint(255, 150); image(questionBlock, 370, 10); tint(255, 255);
      }
    }
  }
